...
pub fn multiply_w_vec_boxed<F: PseudoField>(
    ...
    let new_result: Vec<F> = (0..size)
        ...
        .map(|row| {
            ...
            for (column, value) in vector.iter().enumerate().take(size).skip(row) {
                let mut factor = if row == column {
                    let mut result = getter(row, column);
                    result.add_assign(add);
                    result
                } else {
                ...
                factor.mul_assign(value);
                partial_res.add_assign(&factor);
            }
...
/// Searches new rate values for the rate count of `to_set_rc`, based on the targets generated by the first value from it,
...
/// Computes the Longest Core Series based on the adjancy matrix and the total exit rates of each state.
/// Uses Khan's algorithm for a topographical sort of a DAG to set an order of the states,
/// then it goes through them, and each state updates the values of its sucessors.
pub fn max_value_counts<'a, F: PseudoField>(
    adj_matrix: &HashMap<usize, Vec<usize>>,
    vals: &'a [F],
) -> HashMap<&'a F, i32> {
    let size = adj_matrix.len();
    let mut in_count = vec![0; size + 1];

    adj_matrix.values().for_each(|xs| {
        xs.iter().for_each(|v| {
            in_count[*v] += 1;
        })
    });
    ...
    while let Some(u) = queue.pop_front() {
        topo_order.push(u);
        for v in adj_matrix.get(&u).unwrap_or(&vec![]) {
            in_count[*v] -= 1;
            // Khan's Algorithm:
            // Add to queue only when all parents have already been visited
            if in_count[*v] == 0 {
                queue.push_back(*v);
            }
        }
    ...
    let mut dp = vec![HashMap::new(); size + 1];

    for u in &topo_order {
        if *u == size {
        } else {
            *dp[*u].entry(&vals[*u]).or_insert(0) += 1;
            let current = dp[*u].clone(); // This clone gets expensive for each state.
            for v in adj_matrix.get(u).unwrap_or(&vec![]) {
                for (k, count) in &current {
                    dp[*v]
                        .entry(k)
                        .and_modify(|x| *x = i32::max(*x, *count))
                        .or_insert(*count);
                }
            }
        }
