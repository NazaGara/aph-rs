...
pub struct Float32(rug::float::OrdFloat);

impl Display for Float32 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mut val = self.0.as_float().clone();
        val.set_prec_round(23, rug::float::Round::Nearest);
        write!(f, "{:.8}", val)
    }
}

impl FromRational for Float32 {
    fn from_rational(nominator: &str, denominator: &str) -> Self {
        ...
        Self(rug::Float::with_val(23, nominator / denominator).into())
    }
}

impl ToRational for Float32 {
...
impl Add for Float32 {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        Self(OrdFloat::from(
            self.0.as_float().add(rhs.0.as_float()).complete(23),
        ))
    }
}

impl num_traits::Zero for Float32 {
    fn set_zero(&mut self) {
        *self = Float32(rug::Float::with_val(23, 0.0).into());
    }

    fn zero() -> Self {
        Self(rug::Float::with_val(23, 0.0).into())
    }
...
impl Mul for Float32 {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        Self(OrdFloat::from(
            self.0.as_float().mul(rhs.0.as_float()).complete(23),
        ))
    }
}

impl num_traits::One for Float32 {
    fn is_one(&self) -> bool
    where
        Self: PartialEq,
    {
    ...
    fn one() -> Self {
        Self(rug::Float::with_val(23, 1.0).into())
    }
    fn set_one(&mut self) {
        *self = Float32(rug::Float::with_val(23, 1.0).into());
    }
}

impl From<f64> for Float32 {
    fn from(value: f64) -> Self {
        Self(rug::Float::with_val_64(23, value).into())
    }
}

impl From<Float32> for f64 {
    fn from(float: Float32) -> Self {
...
impl SparseField for Float32 {
    ...
    fn inv(&mut self) {
        *self = Self(rug::Float::with_val_64(23, 1.0.div(rug::Float::from(self.0.clone()))).into())
    }
...
impl Sub for Float32 {
    ...
    fn sub(self, rhs: Self) -> Self::Output {
        ...
        neg_rhs.neg_assign();
        Self(OrdFloat::from(
            self.0.as_float().add(neg_rhs.0.as_float()).complete(23),
        ))
    }
}